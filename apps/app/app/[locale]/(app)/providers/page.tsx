      ...
      <CardContent className="space-y-4">
          ...
          {tabs.map((t) => (
            <button
              ...
              className={`flex-1 rounded-md py-1.5 text-xs font-semibold transition-colors ${
                tab === t ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground/80'
              }`}
            >
        ...
        {tab === 'llm' && agentType === 'AI' && (
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              <Label className="text-xs">API Key override <span className="text-muted-foreground/70">(optional)</span></Label>
              <div className="relative">
                <Input
                  ...
                  className="pr-10"
                />
                <button type="button" onClick={() => setShowApiKey((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground/70 hover:text-foreground/80">
  ...
  if (!provider) {
    return (
      <div className="min-h-full p-8">
        <h1 className="mb-1 text-2xl font-bold text-foreground">Provider Dashboard</h1>
        <p className="mb-8 text-sm text-muted-foreground">Publish agents to the marketplace and earn revenue</p>
        ...
        {!showRegisterForm ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border bg-card py-24 text-center">
            ...
            <p className="font-semibold text-foreground/80">Not registered as a provider</p>
            <p className="mt-1 text-sm text-muted-foreground/70">Register to publish agents and earn from the marketplace.</p>
            <Button className="mt-6" onClick={() => setShowRegisterForm(true)}>
              <Plus size={14} className="mr-1.5" /> Become a provider
            </Button>
          </div>
        ) : (
          <div className="max-w-md rounded-2xl bg-card p-6 shadow-sm ring-1 ring-border">
            <h2 className="mb-4 font-semibold text-foreground">Register as a provider</h2>
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                <Label>Bio <span className="text-muted-foreground/70">(optional)</span></Label>
    ...
    <div className="min-h-full p-8">
      ...
      >
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Provider dashboard</p>
        </div>
        {earnings && (
      ...
      >
        <div className={`flex items-center gap-3 rounded-2xl border p-4 ${hasDpa ? 'border-emerald-800 bg-emerald-950/50' : 'border-amber-200 bg-amber-50'}`}>
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground">{hasDpa ? `Accepted v${provider.activeDpaVersion}` : 'Not accepted — required to publish'}</p>
          </div>
          {hasDpa ? (
        ...
        <div className={`flex items-center gap-3 rounded-2xl border p-4 ${hasKyc ? 'border-emerald-800 bg-emerald-950/50' : 'border-border bg-card'}`}>
          <div className={`flex h-9 w-9 items-center justify-center rounded-xl ${hasKyc ? 'bg-emerald-100' : 'bg-muted'}`}>
            <AlertTriangle size={18} className={hasKyc ? 'text-emerald-600' : 'text-muted-foreground/70'} />
          </div>
          <div>
            <p className="text-sm font-semibold text-foreground">Identity Verification (KYC)</p>
            <p className="text-xs text-muted-foreground">{hasKyc ? 'Verified' : 'Pending — contact support to verify'}</p>
          </div>
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="font-semibold text-foreground">Marketplace Agents</h2>
          <Button size="sm" variant="outline" onClick={() => setShowAgentForm((v) => !v)} className="bg-card">
        ...
        {agentsLoading ? (
          ...
          <div className="rounded-2xl border-2 border-dashed border-border bg-card py-12 text-center text-sm text-muted-foreground/70">
          ...
          <div className="space-y-3">
            {agents.map((agent, i) => (
              <motion.div
                ...
                className="group flex items-center gap-4 rounded-2xl bg-card p-4 shadow-sm ring-1 ring-border"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    <span className="rounded-full bg-muted px-2 py-0.5 text-[11px] font-medium text-muted-foreground">{agent.type}</span>
                    <span className="rounded-full bg-muted px-2 py-0.5 text-[11px] font-medium text-muted-foreground">{agent.pricingModel.replace('_', ' ')}</span>
