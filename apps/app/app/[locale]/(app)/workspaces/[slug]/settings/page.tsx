    ...
    <div className="min-h-full p-8">
      ...
      >
        <Link
          ...
          className="mb-4 flex w-fit items-center gap-1.5 rounded-lg px-2 py-1 text-sm text-muted-foreground/70 hover:bg-muted hover:text-foreground/80 transition-colors"
        >
          <ArrowLeft size={14} /> Back to workspace
        </Link>
        <h1 className="text-2xl font-bold text-foreground">{workspace?.name ?? '…'} — Settings</h1>
        <p className="mt-1 text-sm text-muted-foreground">Manage LLM provider keys and workspace configuration</p>
      </motion.div>
      ...
      >
        ...
        <div className="mb-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground/70">Shared keys for all agents in this workspace</p>
            </div>
          </div>
        ...
        {showForm && (
          <motion.div
            initial={{ opacity: 0, y: -8 }}
            animate={{ opacity: 1, y: 0 }}
            className="mb-4 rounded-2xl bg-card p-5 shadow-sm ring-1 ring-border space-y-4"
          >
            <h3 className="font-medium text-foreground">Add Provider API Key</h3>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs">Label <span className="text-muted-foreground/70">(optional)</span></Label>
        ...
        {isLoading ? (
          <div className="space-y-3">
            {[...Array(2)].map((_, i) => <div key={i} className="h-16 animate-pulse rounded-2xl bg-muted" />)}
          </div>
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border bg-card py-16 text-center">
            <ShieldCheck size={28} className="mb-3 text-muted-foreground/50" />
            <p className="text-sm font-medium text-muted-foreground">No provider keys configured</p>
            <p className="mt-1 text-xs text-muted-foreground/70">Add API keys to allow agents to use LLM providers.</p>
            ...
            {providerKeys.map((k, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex items-center gap-4 rounded-2xl bg-card px-5 py-4 shadow-sm ring-1 ring-border"
                >
                  <KeyRound size={15} className="shrink-0 text-muted-foreground/70" />
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="mt-0.5 font-mono text-xs tracking-widest text-muted-foreground/50">••••••••••••••••</p>
                  </div>
      ...
      >
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground/70">API keys for agent sandbox providers (used for real code execution)</p>
          </div>
        </div>

        <div className="rounded-2xl bg-card px-5 py-4 shadow-sm ring-1 ring-border space-y-4">
          <div className="flex items-center justify-between">
            ...
            {e2bKey ? (
              ...
              <span className="rounded-full bg-muted px-2 py-0.5 text-[11px] font-semibold text-muted-foreground">Not set</span>
            )}
          </div>
