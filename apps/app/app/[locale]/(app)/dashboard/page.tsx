...
const AVATAR_GLOWS = [
  'shadow-[0_0_24px_var(--accent-workspace-glow)]',
  'shadow-[0_0_24px_var(--accent-workspace-glow)]',
  'shadow-[0_0_24px_var(--accent-workspace-glow)]',
  'shadow-[0_0_24px_var(--accent-workspace-glow)]',
  'shadow-[0_0_24px_var(--accent-workspace-glow)]',
    ...
    <div className="min-h-full p-8">
      ...
      >
        <div>
          ...
          <h1 className="text-4xl font-bold text-[var(--text-primary)]" style={{ fontFamily: 'var(--font-syne), system-ui, sans-serif' }}>
        ...
        <Link
          ...
          className={buttonVariants({ className: 'shadow-[0_0_20px_var(--accent-workspace-glow)] hover:shadow-[0_0_28px_var(--accent-workspace-glow-hover)] transition-shadow' })}
        >
      ...
      {isLoading ? (
        ...
        >
          <div className="mb-6 relative flex h-20 w-20 items-center justify-center">
            ...
            <div className="absolute -top-2 -right-2 h-6 w-6 rounded-lg bg-[var(--accent-agent-bg)] border border-[var(--accent-agent-border)]" />
          ...
          <p className="font-semibold text-[var(--text-primary)] text-lg" style={{ fontFamily: 'var(--font-syne), system-ui, sans-serif' }}>{t('noWorkspaces')}</p>
          <p className="mt-2 text-sm text-[var(--text-secondary)] max-w-xs">{t('noWorkspacesDesc')}</p>
          <Link
            href={`/${locale}/workspaces/new`}
            className={buttonVariants({ className: 'mt-7 shadow-[0_0_20px_var(--accent-workspace-glow)] hover:shadow-[0_0_28px_var(--accent-workspace-glow-hover)] transition-shadow' })}
          >
              ...
              <Link href={`/${locale}/workspaces/${ws.slug}`}>
                <div className="group relative overflow-hidden rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-5 shadow-lg shadow-black/30 transition-all hover:border-[var(--border-strong)] hover:shadow-[0_8px_32px_black/50]">
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <ArrowRight size={15} className="text-[var(--text-muted)] group-hover:text-[var(--text-secondary)] transition-colors mt-0.5" />
                  </div>
                  <div className="mt-3">
                    <p className="font-semibold text-[var(--text-primary)]" style={{ fontFamily: 'var(--font-syne), system-ui, sans-serif' }}>{ws.name}</p>
                    {(ws as any).description && (
                      <p className="mt-0.5 text-sm text-[var(--text-secondary)] line-clamp-1">{(ws as any).description}</p>
                    )}
                  </div>
