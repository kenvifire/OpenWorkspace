...
export class NotificationsService {
  ...
  private buildEmailHtml(type: NotificationType, data: NotificationData): string {
    const projectPath = data.projectId ? `/projects/${data.projectId}/board` : '';
    const boardUrl = `${this.config.get('WEB_URL') ?? 'http://localhost:3000'}/en/workspaces/${data.workspaceSlug}${projectPath}`;
